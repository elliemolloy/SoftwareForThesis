...
procedure CheckScanRange(Name,Units:string;EStart,EEnd,EStep:TEdit;Min,Max:Double;
					var Valid:Boolean);
...
procedure CheckScanRange(Name,Units:string;EStart,EEnd,EStep:TEdit;Min,Max:Double;var Valid:Boolean);
var
	m1,m2:string;
	step:Double;
begin
	m1:=Name+' start must be a real number.';
	m2:=Name+' start must be from '+FloatToStr(Min)+Units+' to '+FloatToStr(Max)+Units+'.';
	ValidDlgReal(EStart,'[',Min,Max,']',False,m1,m2,Valid);
	if Valid then
	begin
		m1:=Name+' end must be a real number.';
		m2:=Name+' end must be from '+FloatToStr(Min)+Units+' to '+FloatToStr(Max)+Units+'.';
		ValidDlgReal(EEnd,'[',Min,Max,']',False,m1,m2,Valid);
	end;
	if Valid then
	begin
		m1:=Name+' step must be a real number.';
		m2:='';
		step:=ValidDlgReal(EStep,'i',0,0,'i',False,m1,m2,Valid);
		if valid and (step=0) then
		begin
			Valid:=False;
			m2:=Name+' step must not be 0'+Units+'.';
			Application.MessageBox(PChar(m2),'Invalid Range',mb_IconStop or mb_Ok);
			EStep.SelectAll;
			EStep.SetFocus;
		end;
	end;
